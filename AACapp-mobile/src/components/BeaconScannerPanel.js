  ...
  return (
    <View style={styles.beaconPanelWrap}>
      <View style={styles.beaconPanelHeader}>
        ...
        <TouchableOpacity
          style={[styles.beaconScanButton, isScanning && styles.beaconScanButtonStop]}
      ...
      <Text style={styles.beaconPanelHint}>
        This tab is only for scan verification. It does not update room selection.
      </Text>

      {!!error && <Text style={styles.beaconErrorText}>{error}</Text>}

      <ScrollView style={styles.beaconListScroll} contentContainerStyle={styles.beaconList}>
        {safeDevices.length === 0 ? (
          ...
          safeDevices.map((device) => (
            <View key={device.id} style={styles.beaconCard}>
              ...
              <Text style={styles.beaconMeta}>RSSI: {formatRssi(device.rssi)}</Text>
              {typeof device.txPowerLevel === 'number' && (
                <Text style={styles.beaconMeta}>Tx Power: {device.txPowerLevel} dBm</Text>
              )}
              {!!device.iBeaconUuid && (
                <Text style={styles.beaconMeta}>UUID: {device.iBeaconUuid}</Text>
              )}
              {typeof device.iBeaconMajor === 'number' && (
                <Text style={styles.beaconMeta}>Major: {device.iBeaconMajor}</Text>
              )}
              {typeof device.iBeaconMinor === 'number' && (
                <Text style={styles.beaconMeta}>Minor: {device.iBeaconMinor}</Text>
              )}
            </View>
          ))
        )}
